...
function username() {
    ...
    if (username == "") {
        ...
        usernameFlag = false;
        usernameError.innerText = " ✘用户名可以包含中文、大小写字母、数字及下划线";
    } else {
        usernameError.innerHTML = "<p>&nbsp;</p>";
        usernameFlag = true;
    }
    usernameError.style.paddingBottom = "4px";
...
function password(a) {
    ...
    var patt1 = /^[a-zA-Z0-9@_]{6,40}$/;    //可以包含六位以上的字母数字@和下划线

    var patt2 = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,}$/;  //包含字母和数字的组合

    if (password === "") {
        passwordError.innerHTML = "密码尽量包含大小字母以及数字";
    } else if (patt2.test(password)) {
        passwordError.innerHTML = "密码等级：强";
        passwordFlag = true;
    } else if (patt1.test(password)) {
        passwordError.innerHTML = "密码等级：中";
        passwordFlag = true;
    } else {
        passwordError.innerHTML = "密码尽量包含大小字母以及数字";
    }
...
function repassword() {
    ...
    // alert("password:"+password+";repassword"+repassword);
    //错误提示DOM
    var repasswordError = document.getElementById("repasswordError");
    // alert(repasswordError);
...
function phone() {
    ...
    if (!patt.test(phone)) {
        cellFlag = false;
        // phoneError.innerText = "✘请输入正确的电话号码！";
    } else {
...
function register() {
    ...
    var password = document.getElementById("password").value.replace(/\s+/g, "");
    // 重新输入密码
    var repassword = document.getElementById("repassword").value.replace(/\s+/g, "");
    // 手机号
    var phone = document.getElementById("phone").value.replace(/\s+/g, "");
    //邮箱
    var email = document.getElementById("email").value.replace(/\s+/g, "");
    ...
    //手机号登录
    var x = cellFlag && passwordFlag && repasswordFlag && confirmFlag;
    //邮箱登录
    var y = emailFlag && passwordFlag && repasswordFlag && confirmFlag;

    if (x || y) {
        $.ajax({
            type: 'POST',
            async: true,
            url: "/register", //要访问的后台地址
            data: {
                username: username,
                password: password,
